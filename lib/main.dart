...
Future<void> showCallkitIncoming(
  ...
  try {
    debugPrint(
      'Showing CallKit incoming: uuid=$uuid, callerId=$callerId, channelName=$channelName, userId=$userId',
    );
  ...
  try {
    ...
    if (message.data['type'] == 'call') {
      ...
      if (channelName.isEmpty || int.tryParse(userId) == null) {
        debugPrint(
          'Error: Invalid channelName or userId in background handler',
        );
        return;
      }
      final uuid = Uuid().v4();
  ...
  void _handleForegroundMessages() {
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if (message.data['type'] == 'call') {
        ...
        if (channelName.isEmpty || int.tryParse(userId) == null) {
          debugPrint(
            'Error: Invalid channelName or userId in foreground message',
          );
          return;
        }
        final uuid = Uuid().v4();
      ...
      switch (event.event) {
        ...
        case Event.actionCallTimeout:
          ...
          final params = CallKitParams(
            id: _currentCallUuid!,
            nameCaller:
                'User ${event.body['nameCaller']?.replaceFirst('User ', '') ?? 'Unknown'}',
